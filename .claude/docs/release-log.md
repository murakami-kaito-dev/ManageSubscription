...
- かんたん追加シートの「手入力する」を**フローティング（ホバー様）**にして上方へ移動
...
- **「＋」を「かんたん追加」シートに変更**。人気サブスク20件（公式価格つき）＋固定費8件を
  タイルで並べ、タップ→プラン選択（固定費は金額入力）→追加、で 名前・通貨(JPY)・色・
  プリセットアイコン・**通知(1日前/3日前の9:00)**・開始日(今日・変更可) まで自動設定。
  一覧に無いものは「手入力する」から従来フォームへ。上限チェックは従来どおり前段で適用
- **クイック追加カタログをサーバー配信**（GitHub 公開リポジトリ `submana-catalog` の
  catalog.json）。価格改定は JSON を push するだけで全ユーザーに反映（アプリ更新不要）。
  取得失敗時は 前回取得値→同梱アセット の二段フォールバック。詳細は
...
v2.2.1 (build 18)（セッション復元/ホイールハプティック/起動画像の色段差解消）は
2026-08-19 に**審査提出したが、同日 開発者都合で取りやめ**（この v2.3.0 に内容ごと
巻き取って一本化するため）。**App Store で公開されることはない**。
タグ `v2.2.1` はコード追跡用に**残置**するが、公式リリースには対応しない旨をここに明記
（タグ注釈にも同旨を追記済み）。2.2.1 の3機能は build 19/20/21 の系譜に含まれている。

## 2.3.0 (build 20) — 2026-08-19 · TestFlight

build 19 に、カレンダーのスクロール操作性の修正を加えたもの（バージョンは据え置き）。

- **fix**: カレンダーの上からでも画面全体を上下スクロールできるように
  （`availableGestures` を `horizontalSwipe` に限定。既定の `all` は縦スワイプを
  表示形式切替として横取りするが、月表示のみのため何も起きずスクロールを妨げていた）。
  前後月の横スワイプは従来どおり

**配信**: TestFlight にアップロード（Delivery UUID `a00cd155-d42e-4e97-a01a-1bf20e37d81e`）。
検証・アップロードとも `SUCCEEDED with no errors`
**状態**: 審査未提出（TestFlight で確認中）。analyze 0 / test 95件グリーン

## 2.3.0 (build 19) — 2026-08-19 · TestFlight

- **カレンダーの日付セルにサービス名チップを表示**（色ドットを置換）。「いつ・何が」支払われるか
  一瞥で分かるようにした。アイテム色の淡い地＋濃い文字、長い名前は末尾 `…` 省略、
  1日3件以上は2件＋「+N」。下部の選択日一覧は従来どおり。実装 `widgets/day_payment_chips.dart`
  （検証時に比較したアイコン案・週表示案は不採用として削除）

**配信**: TestFlight にアップロード（Delivery UUID `f6bbeabb-4e75-47fd-9e1f-3a0fbb3a4399`）。
検証・アップロードとも `SUCCEEDED with no errors`
**状態**: 審査未提出（TestFlight で実データ確認 → 問題なければ審査提出）。
analyze 0 / test 95件グリーン
**メモ**: 2.2.1 (build 18) が審査中のため、そのトレインに build を足さず 2.3.0 に上げた
（2.2.1 承認時にトレインが閉じ、同版の追加ビルドが使えなくなるのを避けるため）

## 2.2.1 (build 18) — 2026-08-19 · App Store Connect（審査中）

2.2.0 の直後に出す小改修（UX 3点）。

- **セッション復元**: iOS のメモリ圧力 kill（jetsam）後の再起動で、最後に開いていた
  タブと編集途中の入力（下書き）に自動復帰（30分の鮮度窓つき）。詳細は
  [architecture.md](architecture.md) の `sessionRestoreProvider`
- **ホイールに選択ハプティック**: 時刻・日前・カスタム周期の3ホイールでカリカリと振動
- **起動画像の色段差を解消**: アイコンの白背景を起動画面の背景色 #ECE9E2 に正規化

**配信**: App Store Connect にアップロード（Delivery UUID `ae467ff4-c8ae-414a-829a-b021cf588349`）。
build 18 処理 VALID → バージョン 2.2.1 作成 → What's New 設定＋プロモは 2.2.0 から引き継ぎ →
build 18 紐付け → 審査提出（reviewSubmission `49b555e1…`）。タグ `v2.2.1` = `8062058`
**状態**: **WAITING_FOR_REVIEW（審査中）**。`releaseType=AFTER_APPROVAL`（承認後に自動配信）。
analyze 0 / test 91件グリーン。購入経路コードは 2.2.0 から差分ゼロ（公開中の版と同一）

## 2.2.0 (build 17) — 2026-08-18 · App Store（公開済み・旧版）
...
**状態**: **公開済み（`READY_FOR_SALE`）**。2026-08-19 に承認・自動配信を ASC API で確認 → その後 2.2.1 を提出
