...
- メモ、状態（停止中トグル・編集時）、削除
- 詳細設定（折りたたみ）: アイコン（**プリセットアイコンから選ぶ**/写真/カメラ/文字＋背景色。プリセットは解約ガイドと同じスケッチ風SVG）、カテゴリ、支払い方法、コスパ

## かんたん追加（`features/subscription/widgets/quick_add_sheet.dart`）
- ホームの「＋」（上限チェック通過後）で開くボトムシート。**人気サブスク20件＋固定費8件**の
  タイル（アイコン＋名前）と、下部固定の**「手入力する」**ボタン（→従来の新規追加フォーム）
- タイルタップ → 確認シート: **プラン選択（公式価格・単一プランは選択済み）**、
  固定費は代わりに**金額入力**。初回支払日=今日（タップで変更可）。
  「追加する」で即保存 → 通貨JPY・色・プリセットアイコン・**通知（1日前/3日前の9:00）**が自動設定
- カタログは `CatalogService` がサーバー配信（[architecture.md](architecture.md) の
  「クイック追加カタログ」参照）。編集は従来の編集画面でいつでも可能
