...
- 各行と詳細シートに**サービスごとのプリセットアイコン**（`assets/preset_icons/<id>.svg`・スケッチ風・ロゴ非使用）を表示
- カテゴリー順: 登録先（App Store / Google Play）→ 動画 → 音楽 → 仕事・AI・クラウド → ゲーム → 読書・雑誌・生活 → 学び・教育 → フィットネス・健康 → このアプリ（計48サービス）。**登録先が最上段**（アプリ内課金の解約先はサービスのサイトではないため）
...
- メモ、状態（停止中トグル・編集時）、削除
- 詳細設定（折りたたみ）: アイコン（**プリセットアイコンから選ぶ**/写真/カメラ/文字＋背景色。プリセットは解約ガイドと同じ48種のスケッチ風SVG）、カテゴリ、支払い方法、コスパ
