...
| カテゴリー登録数 | 5件 | 無制限 |
| 支払い方法登録数 | 5件 | 無制限 |
| 支払い前通知ルール | **1サブスクにつき5件**（全員共通・課金で増えない） | 同左 |
| カテゴリー別/支払い方法別の分析 | **✓（無料）** | ✓ |
| 並べ替え・テーマカラー・**アイコン画像** | ✓ | ✓ |
| CSVエクスポート / CSVインポート / CSV書式サンプル | ✗ | ✓ |
...
カテゴリ/支払い方法は削除不可アイコン表示。**サブスク行をタップすると読み取り専用プレビュー**
（`SubscriptionFormScreen(readOnly:true)`：全項目グレーアウト・詳細設定も常時表示・保存ボタン無し）で
削除前に内容を確認できる。
...
- CTA はボタン本文（短文で見切れない）＋下に価格キャプション。**トライアル対象外なら「2週間無料」を出さない**
  （`PurchaseService.introTrialEligible`。Apple は導入オファーをグループ×Apple ID で1回のみ付与＝**2週間無料は一度きり**）
- **購入成功で祝福ダイアログが自動で閉じ、ペイウォールも自動で閉じて元画面へ戻る**（手動✗不要）
- 下部に無料 vs プレミアムの比較表（全✓行は非掲載）。「無料プランのまま使う」で閉じる。復元・規約・ポリシー
